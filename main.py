...
def info(cls, status):
    ...
    if nationality_req and lifeboat_req != "None":
        return redirect(url_for("results", cls=cls, status=status, nationality=nationality_req,
                                lifeboat=lifeboat_req))  # send all relevant info to 'results'

    return render_template("info.html", nation_table=nationalityBreakdown, lb_table=lifeBoatBreakdown, cls=cls,
                           status=status)  # renders page depending on prior info
...
def results(cls, status, nationality, lifeboat):
    chance = classGenderTable[cls][status] * nationalityBreakdown[cls][nationality] * lifeBoatBreakdown[status][cls][
        lifeboat]  #
    return str(chance)
